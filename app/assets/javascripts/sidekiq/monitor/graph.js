...
(function() {
  ...
  SidekiqMonitor.Graph = (function() {
    function Graph() {
      this.object_values = bind(this.object_values, this);
      this.array_difference = bind(this.array_difference, this);
      this.standardize_graphs_config = bind(this.standardize_graphs_config, this);
      this.render_graph = bind(this.render_graph, this);
      this.render = bind(this.render, this);
      this.start_polling = bind(this.start_polling, this);
    ...
    Graph.prototype.render = function() {
      return d3.json(SidekiqMonitor.settings.api_url('jobs/graph'), (function(_this) {
        return function(data) {
          var graph_config, graphs_config, index, queues, results;
          ...
          for (index in graphs_config) {
            ...
            results.push(_this.render_graph(data, graph_config));
          }
          return results;
        };
      })(this));
    ...
    Graph.prototype.render_graph = function(data, graph_config) {
      var j, label, legend, legend_box_height, legend_box_width, legend_x, legend_y, len, queue, queue_status_counts, queues, queues_status_counts, rect, ref, rule, statuses;
      ...
      ref = data.queues_status_counts;
      for (j = 0, len = ref.length; j < len; j++) {
        queue_status_counts = ref[j];
    ...
    Graph.prototype.standardize_graphs_config = function(graph_config, queues) {
      var graph, graphs, j, len, matched_queues, pattern, pattern_queues, queue, re, remaining_queues, title;
      graphs = [];
      matched_queues = [];
      for (pattern in graph_config) {
        ...
        if (pattern === 'OTHER' || pattern === 'ALL') {
          ...
          pattern_queues = [];
          for (j = 0, len = queues.length; j < len; j++) {
            queue = queues[j];
